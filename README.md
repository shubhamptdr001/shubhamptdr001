<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&pause=1000&color=00C2FF&center=true&vCenter=true&width=700&lines=Hi+I'm+Shubham+Patidar;MCA+Student+at+NIT+Patna;AI+/+ML+Engineer;Full+Stack+Developer+%7C+React+%7C+MySQL;Always+Learning+New+Technologies" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=shubhamptdr001&label=Profile%20Views&color=0e75b6&style=flat"/>
</p>
🛠 Tech Stack
<p align="left">
<img src="https://skillicons.dev/icons?i=java,spring,react,nodejs,mysql,mongodb,git,github,docker,postman,html,css,js,cpp,python,vscode,idea" />
</p>
🏆 GitHub Trophies

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=shubhamptdr001&theme=algolia&no-frame=true&margin-w=15"/>
</p>

<p align="center">
<a href="https://www.linkedin.com/in/shubham-patidar-479417229/">
<img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin"/>
</a>

<a href="mailto:shubhamptdr619@gmail.com">
<img src="https://img.shields.io/badge/Gmail-red?style=for-the-badge&logo=gmail"/>
</a>
</p>

## 🚀 About Me

🎓 I'm pursuing a **Master of Computer Applications (MCA)** at **National Institute of Technology (NIT) Patna** (2024–2027).

I enjoy building scalable software solutions using **AI/ML, Mern Development, Java, and MySQL**. I love transforming ideas into practical applications and continuously improving my knowledge of software engineering, backend development, and data structures.

* 🎓 MCA @ National Institute of Technology Patna
* 💻 Interested in Software Development & Backend Engineering
* 🌱 Currently learning **Spring Boot, Microservices, Docker, and AWS**
* 🧠 Strong foundation in **Data Structures & Algorithms**
* ⚡ Passionate about writing clean, maintainable, and efficient code
* 🤝 Open to Internship and Full-Time Software Engineering opportunities

---

## 🛠 Tech Stack

### 💻 Languages

```text
Java • C++ • Python • JavaScript • SQL • Bash
```

### ⚙️ Backend

```text
Spring Boot • Spring MVC • JDBC • Node.js • Express.js • REST APIs
```

### 🎨 Frontend

```text
React.js • HTML5 • CSS3 • Tailwind CSS • Bootstrap • Vite
```

### 🗄 Databases

```text
MySQL • MongoDB • PostgreSQL • SQLite
```

### ☁️ Tools & Technologies

```text
Git • GitHub • Docker • Postman • Maven • VS Code • IntelliJ IDEA • Eclipse • LaTeX
```

### 🤖 AI / ML

```text
OpenAI API • LangChain • RAG • Semantic Search • TensorFlow • PyTorch • OpenCV
```

---

## 📊 GitHub Analytics

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=shubhamptdr001&show_icons=true&theme=tokyonight"/>
  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=shubhamptdr001&theme=tokyonight"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=shubhamptdr001&layout=compact&theme=tokyonight"/>
</p>

---

## 💡 Developer Quote

> *"Code. Learn. Build. Repeat."*

---

⭐ If you like my work, consider giving a star to my repositories!

